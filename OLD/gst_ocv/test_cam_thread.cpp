...
void test_gst_cam(){
	...
	VideoCapture cap1("v4l2src device=\"/dev/video0\" ! video/x-raw,width=1920,height=1080,format=BGR ! appsink");
...
void test_gst_cam_1(){
	...
	//VideoCapture cap1("v4l2src device=/dev/video2 ! video/x-raw, framerate=30/1, width=1920, height=1080, format=RGB ! videorate ! videoconvert ! appsink");
	VideoCapture cap2("v4l2src device=\"/dev/video2\" ! video/x-raw,width=1920,height=1080,format=BGR ! appsink");
	//I assigned a  new space for each camera to store a frame in one second 
	Mat Webcam2;
	std::cout << "thread " << this_id << "\n";
	while (true){
...
int main()
{
	...
	std::thread t1(test_gst_cam);  // spawn new thread that calls foo()
	//std::thread t2(test_gst_cam_1);
